...
const Productivity = ({
  ...
  const nominalPendapatan1 =
    detailMerchant && detailMerchant.map((row) => Number(row[8].stringValue));
  const nominalPendapatan2 =
    detailMerchant && detailMerchant.map((row) => Number(row[10].stringValue));
  const nominalPendapatan3 =
    detailMerchant && detailMerchant.map((row) => Number(row[12].stringValue));
  const nominalPendapatan4 =
    detailMerchant && detailMerchant.map((row) => Number(row[14].stringValue));
  const nominalPendapatan5 =
    detailMerchant && detailMerchant.map((row) => Number(row[16].stringValue));
  const nominalPendapatan6 =
    detailMerchant && detailMerchant.map((row) => Number(row[18].stringValue));
  const nominalPendapatan7 =
  ...
  return (
    <>
      <Widget
        styleName={`gx-card-full gx-bg-${itemList.status}`}
        extra={
          <CloseCircleFilled style={{ fontSize: "25px" }} onClick={cancel} />
        }
      >
        <div className="gx-text-center gx-px-3 gx-pt-5 ">
          ...
          <span><i className="icon icon-chat gx-fs-xxl gx-text-dark"/><br/>{itemList.email}</span> */}
            ...
            <Col span={8}>
              <span>
                <i className="icon icon-chat gx-fs-xxl gx-text-dark" />
                <br />
                {itemList.email}
              </span>
            </Col>
